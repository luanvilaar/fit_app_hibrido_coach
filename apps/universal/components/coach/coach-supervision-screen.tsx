...
const supervisionTabs: Array<{ label: string; testID: string; value: Tab }> = [
  { label: "Atletas", testID: "coach-supervision-tab-athletes", value: "athletes" },
  { label: "Equipes", testID: "coach-supervision-tab-teams", value: "teams" }
];

function describeSupervisionError(cause: unknown) {
  if (typeof cause === "object" && cause !== null && "code" in cause && cause.code === "PGRST202") {
    return "O acompanhamento ainda não foi disponibilizado no servidor. A atualização do banco precisa ser aplicada antes de continuar.";
  }
  return describeBackendError(cause);
}
...
function useRoster() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(describeSupervisionError(cause)); setAthletes([]); setTeams([]);
...
export function CoachSupervisionScreen() {
  ...
  const { colors, styles } = useCoachSupervisionStyles();
  ...
  return <View style={styles.page} testID="coach-supervision-screen">
    ...
    <AnimatedTabBar
      ...
      label="Acompanhamento por atleta ou equipe"
      onChange={setTab}
      options={supervisionTabs}
      testID="coach-supervision-tabs"
      value={tab}
    />
    ...
    {!isLoading && !error && (
      <TabTransitionPanel activeKey={tab} order={["athletes", "teams"]} testID="coach-supervision-tab-panel">
        {tab === "athletes" ? (
          <RosterList athletes={shownAthletes} onAthlete={(id) => router.push(`/app/coach/acompanhamento/atletas/${id}`)} />
        ) : (
          <TeamList teams={shownTeams} onTeam={(id) => router.push(`/app/coach/acompanhamento/equipes/${id}`)} />
        )}
      </TabTransitionPanel>
    )}
  </View>;
...
export function CoachSupervisionAthleteScreen() {
  ...
  useEffect(() => {
    ...
    void createCoachSupervisionRepository(supabase).listAthleteSessions(athleteId, from.toISOString().slice(0, 10), to.toISOString().slice(0, 10)).then(setSessions).catch((cause) => setSessionError(describeSupervisionError(cause))).finally(() => setIsLoading(false));
  }, [athleteId]);
...
export function CoachSupervisionSessionScreen() {
  const { athleteId, sessionId } = useLocalSearchParams<{ athleteId: string; sessionId: string }>();
  const router = useRouter();
  const [workout, setWorkout] = useState<CoachSupervisionWorkout | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const { colors, styles } = useCoachSupervisionStyles();

  useEffect(() => {
    if (!supabase || !athleteId || !sessionId) {
      setError(getSupabaseConfigurationError() ?? "Sessão inválida.");
      setIsLoading(false);
      return;
    }

    void createCoachSupervisionRepository(supabase)
      .getAthleteSession(athleteId, sessionId)
      .then(setWorkout)
      .catch((cause) => setError(describeSupervisionError(cause)))
      .finally(() => setIsLoading(false));
  }, [athleteId, sessionId]);
  ...
  const blocks = readSessionBlocks(workout.session);
  const completed = workout.progress?.completed_block_ids.length ?? 0;

  return (
    <View style={styles.page} testID="coach-supervision-session-screen">
      <Pressable
        accessibilityLabel="Voltar para prescrições do atleta"
        accessibilityRole="button"
        onPress={() => router.back()}
        style={styles.back}
      >
        <Ionicons name="arrow-back" color={colors.textPrimary} size={18} />
        <Text style={styles.backText}>Prescrições</Text>
      </Pressable>

      <View>
        <Text style={styles.eyebrow}>CONSULTA SOMENTE LEITURA</Text>
        <Text style={styles.title}>{getSessionTitle(workout.session)}</Text>
        <Text style={styles.description}>
          {workout.session.scheduled_date} · {workout.progress?.state ?? "não iniciada"} · {completed} de {blocks.length} blocos concluídos
        </Text>
      </View>

      {workout.session.coach_note ? (
        <View style={styles.note}>
          <Text style={styles.noteText}>{workout.session.coach_note}</Text>
        </View>
      ) : null}

      {blocks.length ? (
        <View style={styles.list} testID="coach-supervision-blocks">
          {blocks.map((block) => (
            <View key={block.id} style={styles.block}>
              ...
              {block.body ? (
                <BlockBodyText
                  ...
                  testID={`coach-supervision-block-body-${block.id}`}
                  tone="dark"
                />
              ) : null}
            </View>
        ...
        <Text style={styles.state} testID="coach-supervision-blocks-empty">
          Esta sessão não possui blocos prescritos.
        </Text>
      )}

      <View style={styles.results} testID="coach-supervision-results">
        <Text style={styles.rowTitle}>Resultados registrados</Text>
        {workout.results.length ? (
          workout.results.map((result) => (
            <Text key={result.id} style={styles.rowMeta}>
              Série {result.set_number}: {result.reps ?? "—"} reps · {result.load_kg ?? "—"} kg · {result.completed ? "concluída" : "pendente"}
            </Text>
          ))
        ) : (
          <Text style={styles.rowMeta}>Nenhum resultado registrado pelo atleta.</Text>
        )}
      </View>
      <Text style={styles.readOnly}>Esta visualização não altera o treino ou o progresso do atleta.</Text>
    </View>
  );
}

function Avatar({ name }: { name: string }) {
  const { styles } = useCoachSupervisionStyles();
  return <View style={styles.avatar}><Text style={styles.avatarText}>{name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</Text></View>;
}

function useCoachSupervisionStyles() {
