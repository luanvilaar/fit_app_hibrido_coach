-- Perfil exclusivo coach/owner e acompanhamento observacional de atletas.
-- O owner continua administrando/atuando como coach, mas não recebe mais um papel de atleta implícito.
  ...
  select jsonb_build_object(
    ...
    'coach_team_ids', coalesce((
      ...
      where public.is_platform_owner() or exists (
        ...
        where member_row.team_id = team_row.id and member_row.user_id = auth.uid() and member_row.role = 'coach'
      )
    ), '[]'::jsonb),
    'athlete_team_ids', coalesce((
      select jsonb_agg(member_row.team_id order by member_row.team_id)
      from public.team_members member_row
      where member_row.user_id = auth.uid() and member_row.role = 'athlete'
    ), '[]'::jsonb)
  );
$$;

-- Owner não é membro para efeitos de execução de atleta, mas continua podendo ler/adminstrar
-- equipes via sua capacidade de coach global.
drop policy if exists "team members can read their teams" on public.teams;
create policy "team members and coaches can read their teams"
on public.teams for select to authenticated
using (public.is_team_member(id) or public.is_team_coach(id));

-- Remove apenas a associação atleta da conta de operação. É seguro reaplicar e não toca coaches/owners.
delete from public.team_members member_row
using auth.users account
where member_row.user_id = account.id
  and member_row.role = 'athlete'
  and lower(account.email) = 'l.vilaar@gmail.com';
